...
import nourl.mythicmetalsdecorations.backport.MMItemGroup;
...
public class MythicMetalsDecorations implements ModInitializer {
    ...
    public static final OwoItemGroup MYTHICMETALS_DECOR = MMItemGroup.of(
            RegHelper.id("main"),
                    () -> MythicDecorations.CELESTIUM.getChain().asItem().getDefaultStack()
            )
            .initializer(group -> {
                group.addTab(Icon.of(MythicDecorations.ADAMANTITE.getChain()), "chains", TagKey.of(Registry.ITEM_KEY, RegHelper.id("chains")), ItemGroupTab.DEFAULT_TEXTURE);
                group.addTab(Icon.of(MythicDecorations.MIDAS_GOLD.getChest()), "chests", TagKey.of(Registry.ITEM_KEY, RegHelper.id("chests")), ItemGroupTab.DEFAULT_TEXTURE);
                group.addTab(Icon.of(MythicDecorationsItems.HYDRARGYM_INGOT), "misc", TagKey.of(Registry.ITEM_KEY, RegHelper.id("misc")), ItemGroupTab.DEFAULT_TEXTURE);
